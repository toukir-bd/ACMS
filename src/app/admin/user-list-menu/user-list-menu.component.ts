...
export class UserListMenuComponent extends BaseComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.getUserListMenuData(true, true);
    this.pageHeader = this.fieldTitle['userlist'];
    this.initGridOption();
  }

  getUserListMenuData(isRefresh: boolean, isActive: boolean) {
    try {
      ...
      this.dataSvc.getUserListMenuData(this.spData, isActive).subscribe({
        next: (res: any) => { 
  ...
  gridColumns(): ColumnType[] {
    return [
      { field: "role", header: this.fieldTitle['role'] },
  ...
  refreshGridData() 
  {
    this.getUserListMenuData(true, false);
  }
