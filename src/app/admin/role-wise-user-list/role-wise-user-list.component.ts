...
@Component({
  selector: 'app-role-wise-user-list',
  templateUrl: './role-wise-user-list.component.html',
  providers: [ModalService, UserListModelService, UserListDataService],
  standalone:true,
  imports:[SharedModule]
})
export class RoleWiseUserListComponent extends BaseComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.groupId = this.modalService?.modalData?.groupId;
    this.getUserListByGroupID(true, this.groupId);
    this.pageHeader = this.fieldTitle['userlist'];
    this.initGridOption();
  }

  getUserListByGroupID(isRefresh: boolean, groupId: number) {
    try {
      ...
      this.dataSvc.getUserListByGroupID(this.spData, groupId).subscribe({
        next: (res: any) => { 
  ...
  gridColumns(): ColumnType[] {
    return [
      { field: "groupName", header: this.fieldTitle['role']},
      ...
      { field: "employeeID", header: this.fieldTitle['employeeid'], isMultiselectFilter: true, dataList: this.employeeIDeList, labelField: 'EmployeeID' },
